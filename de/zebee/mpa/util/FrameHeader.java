...
//-----[ static stuff ]-----
	...
	public static final String[] MPEG_NAME = {
		"MPEG2.5",null,"MPEG2","MPEG1"};
	public static final String[] LAYER_NAME = {
		null,"Layer3","Layer2","Layer1"};
	public static final String[] MODE_NAME = {
		"Stereo","J-Stereo","Dual","Mono"};
	...
	/** [mpegID][layerID][bitrateIndex] -> bitrate in kbps, 0=free-format (no support) */
	public static final int[][][] BITRATE_MAP = { // 
		// invalid, layer3, layer2, layer1
...
//-----[ object methods ]-----
	...
	private void decode() {
		...
		emphasis = header32 & 3;
		valid = (mpegID != ILLEGAL_MPEG_ID) && (layerID != ILLEGAL_LAYER_ID)
		&& (bitrateIndex != 0) && (bitrateIndex != 15) && (samplingrateIndex != ILLEGAL_SR);
		if (valid) {
			samplingrateHz = SAMPLING_RATES[samplingrateIndex];
			if (mpegID == MPEG2_ID)  samplingrateHz >>= 1;	// 16,22,48 kHz
			if (mpegID == MPEG25_ID) samplingrateHz >>= 2;	// 8,11,24 kHz
			...
			if (bytesPerSlot>1) frameSize -= frameSize % bytesPerSlot;
			if (padding) frameSize += bytesPerSlot;
		}
	}

	public int getBitrateIndex() { return bitrateIndex; }

	public boolean isCopyrighted() { return copyrighted; }

	public boolean isCrc16used() { return crc16used; }

	public int getEmphasis() { return emphasis; }

	public int getHeader32() { return header32; }
	...
	public boolean isPrivateBitSet() { return privateBitSet; }

	public int getSamplingrateIndex() { return samplingrateIndex; }
	...
	public int getSamplingrateHZ() { return samplingrateHz; }
