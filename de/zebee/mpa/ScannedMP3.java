...
public class ScannedMP3 {
	...
	public ScannedMP3(InputStream ips) throws IOException {
		try {
				...
				for (;;) {
					...
					if (!firstFrameFound) {
						...
						if (xiltFrame.parse(temp,0)) {
							if (xiltFrame.xingTagPresent()) isVBR = true;
							frameCounter--;
							if (xiltFrame.lameTagPresent()) {
						...
						if (frameCounter==0) {
							checkBitRate = false;
							// first music frame. might be a PCUT-tag reservoir-filler frame
							int sie = fh.getSideInfoEnd();
							// a pcut frame contains its tag in the first 10 bytes of the
							// main data section
							boolean pcutFrame = (sie+10<=frameSize)
							&& (temp[sie  ]==0x50)  // P
							&& (temp[sie+1]==0x43)  // C
							&& (temp[sie+2]==0x55)  // U
							&& (temp[sie+3]==0x54); // T
							if (pcutFrame) {
								...
								long t = temp[sie+5]; // fetch 40 bit start sample
						...
						// we don't want the first "music frame" to be checked if it's
						// possibly a PCUT generated reservoir frame
						if (checkBitRate && fh.getBitrateKBPS()!=firstkbps) isVBR = true;
					}
					if (frameCounter>=0) {
			...
			if (firstFrameFound) {
				float framerate = ((float)firstFrameHeader.getSamplingrateHZ()) / firstFrameHeader.getSamplesPerFrame();
			...
			try { ips.close(); } catch (IOException x) {}
		}
	}

	public int getSamplingFrequency() {
		if (firstFrameHeader==null) return 44100;
	...
	public void crop(long startSample, long endSample, InputStream ips, OutputStream ops) throws IOException {
		try {
			...
			{
				int t = 0xFF;
				if (startSample!=0) t &= XingInfoLameTagFrame.MASK_ATH_KILL_NO_GAP_START;
				if (endSample!=getSampleCount()) t &= XingInfoLameTagFrame.MASK_ATH_KILL_NO_GAP_END;
				maskATH = t;
			}
			int firstFrameInclusive = Math.max(0,(int)((startSample+encDelay-minOverlapSamplesStart) / samplesPerFrame));
			int lastFrameExclusive = Math.min(musicFrameCount,(int)((endSample+encDelay+minOverlapSamplesEnd+samplesPerFrame-1) / samplesPerFrame));
			int newEncDelay = encDelay+(int)(startSample-firstFrameInclusive*samplesPerFrame);
			int newEncPadding = (int)((long)(lastFrameExclusive-firstFrameInclusive)*samplesPerFrame-newEncDelay-(endSample-startSample));
			accessFrameRecord(firstFrameInclusive);
			final int needBytesFromReservoir = getBitResPtr();
			int gotBytesFromReservoir = 0;
			int needPreFrames = 0;
			while (firstFrameInclusive-needPreFrames>0 && needBytesFromReservoir>gotBytesFromReservoir && newEncDelay+1152<=4095) {
				needPreFrames++;
			...
			if (needPreFrames>0) {
				...
				resFrameSize = constructReservoirFrame(resFrame,firstFrameHeader,needBytesFromReservoir,newAbsStartSample);
			}
			...
			{ // calculate seek table
				...
				musiLen = ofsXX-ofs00;
				avgBytesPerFrame = (float)(ofsXX-ofs00)/(lastFrameExclusive-firstFrameInclusive);
				avgBytesPerSecnd = avgBytesPerFrame*firstFrameHeader.getSamplingrateHZ()/firstFrameHeader.getSamplesPerFrame();
				avgkbps = avgBytesPerSecnd / 125.f;
				for (int i=0; i<100; i++) {
					int fidx = Math.round(firstFrameInclusive+(i+1.f)/101*(lastFrameExclusive-firstFrameInclusive));
					accessFrameRecord(Math.max(0,fidx));
					seektable[i] = (byte)Math.round((getFrameFileOfs()-ofs00)*255.f/(ofsXX-ofs00));
				}
			}
			byte[] frameBuff = new byte[MPAFrameParser.MAX_MPAFRAME_SIZE];
			int fl = XingInfoLameTagFrame.createHeaderFrame(firstFrameHeader,isVBR,avgkbps,lastFrameExclusive-firstFrameNum,musiLen,50,seektable,newEncDelay,newEncPadding,xiltFrame,frameBuff,0,maskATH);
			...
			if (needPreFrames>0) {
				...
				if (needBytesFromReservoir>0) {
					for (int fi=firstFrameInclusive-needPreFrames; fi<firstFrameInclusive; fi++) {
						...
						ips.skip(tmp-filepos); filepos = tmp;
						fl = getFrameSize();
					...
					System.arraycopy(reservoir,511-needBytesFromReservoir,resFrame,resFrameSize-needBytesFromReservoir,needBytesFromReservoir);
				}
	...
	private static int constructReservoirFrame(byte[] dest, FrameHeader header, int minResSize, long absStartSample) {
		...
		for (int bri=1; bri<=14; bri++) {
			...
			if (mainDataBlockSize>=minResSize) {
				...
				dest[sideInfoEnd+5] = (byte)(absStartSample >>> 32);  // absolute sample start pos
				dest[sideInfoEnd+6] = (byte)(absStartSample >>> 24);  // absolute sample start pos
				dest[sideInfoEnd+7] = (byte)(absStartSample >>> 16);  // absolute sample start pos
				dest[sideInfoEnd+8] = (byte)(absStartSample >>>  8);  // absolute sample start pos
				dest[sideInfoEnd+9] = (byte) absStartSample        ;  // absolute sample start pos
				return frameSize;
			}
		}
		return -1;
	...
	private static void readFully(InputStream ips, byte[] dest, int dofs, int len) throws IOException {
		while (len>0) {
