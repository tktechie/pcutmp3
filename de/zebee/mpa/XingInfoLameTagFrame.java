...
public class XingInfoLameTagFrame {
	...
	private int lameTagOfs = 0; // starting at VBR scale of XingTag !
	private int encDelay;
	private int encPadding;

	public boolean isValid() { return frameSize>0; }
	public void clear() { frameSize = 0; }
	public int getFrameSize() { return frameSize; }
	public boolean xingTagPresent() { return xingTag; }
	public boolean infoTagPresent() { return infoTag; }
	public boolean lameTagPresent() { return lameTag; }
	public int getEncDelay() { return encDelay; }
	public int getEncPadding() { return encPadding; }
	...
	public boolean parse(byte[] data, int ofs) {
		...
		this.lameTag = (data[ofs] == 0x4C)
		&& (data[ofs+1] == 0x41)
		&& (data[ofs+2] == 0x4D)
		&& (data[ofs+3] == 0x45);
		if (!this.lameTag) {
			this.lameTag = (data[ofs] == 0x47)
			&& (data[ofs+1] == 0x4F)
			&& (data[ofs+2] == 0x47)
			&& (data[ofs+3] == 0x4F);
		}
	...
	public static int createHeaderFrame(FrameHeader toBeSimilar, boolean vbr, float kbps, int frameCount, int musicBytes, int vbrScale, byte[] seektable, int encDelay, int encPadding, XingInfoLameTagFrame srcTag, byte[] dest, final int dofs, final int maskATH) {
		...
		System.arraycopy(seektable,0,dest,tagOffset,100); tagOffset+=100;
