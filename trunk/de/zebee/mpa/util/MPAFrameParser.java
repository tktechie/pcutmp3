...
public class MPAFrameParser {
	...
	public static int getMpegFilter(FrameHeader fh) {
		if (fh.isValid()) {
			switch (fh.getMpegID()) {
				case FrameHeader.MPEG1_ID: return FILTER_MPEG1;
				case FrameHeader.MPEG2_ID: return FILTER_MPEG2;
				case FrameHeader.MPEG25_ID: return FILTER_MPEG25;
			}
		}
		return 0;
	}
	...
	public static int getLayerFilter(FrameHeader fh) {
		if (fh.isValid()) {
			switch (fh.getLayerID()) {
				case FrameHeader.LAYER1_ID: return FILTER_LAYER1;
				case FrameHeader.LAYER2_ID: return FILTER_LAYER2;
				case FrameHeader.LAYER3_ID: return FILTER_LAYER3;
			}
		}
		return 0;
	}
	...
	public static int getSamplingrateFilter(FrameHeader fh) {
		if (fh.isValid()) {
			switch (fh.getSamplingrateIndex()) {
				case FrameHeader.SR_32000HZ: return FILTER_32000HZ;
				case FrameHeader.SR_44100HZ: return FILTER_44100HZ;
				case FrameHeader.SR_48000HZ: return FILTER_48000HZ;
			}
		}
		return 0;
	}
	...
	public static int getFilterFor(FrameHeader fh) {
		return getMpegFilter(fh) | getModeFilter(fh)
		| getSamplingrateFilter(fh) | getLayerFilter(fh);
	}
	...
	/**
	 * tries to find the next MPEG Audio Frame, loads it into the
	 * destination buffer (including 32bit header) and returns
	 * a FrameHeader object.
	 * will block until data is available.
	 * will throw EOFException of any other IOException created by the
	 * InputStream object.
	 * set filter to 0 or to any other value using the FILTER_xxx flags
	 ...
	 * tries to find the next MPEG Audio Frame, loads it into the
	 * destination buffer (including 32bit header) and returns
	 * a FrameHeader object. (If destFH is non-null, that object will be
	 * used to store the header infos)
	 * will block until data is available.
	 * will throw EOFException of any other IOException created by the
	 * InputStream object.
	 * set filter to 0 or to any other value using the FILTER_xxx flags
	 ...
	 * @throws IOException
	 */
	public FrameHeader getNextFrame(int filter, byte[] destBuffer, FrameHeader destFH) throws IOException {
		setupFilter(filter);
		for (int z=0; z<4; z++) headBuff[z] = 0;
		int hbPos = 0;
		...
		while (true) {
			...
			if (tmp == -1) { // EOF ?
				if (junkh!=null) {
					for (int i=0; i<4; i++) { // flush headBuff
						if (skipped>=0) junkh.write(headBuff[hbPos]);
						skipped++; hbPos = (hbPos + 1) & 3;
					}
			...
			skipped++;
			hbPos = (hbPos+1) & 3;
			if (headBuff[hbPos] == 0xFF) { // might be the beginning of a sync-word
				int header32 = headBuff[hbPos];
				for (int z=1; z<4; z++) {
					header32 <<= 8;
					header32 |= headBuff[(hbPos+z) & 3];
				}
				if ((header32 & masker) == masked) { // seems to be a frame header
					fh.setHeader32(header32);
					if (fh.isValid() && (((filter & FILTER_STEREO) == 0) || (fh.getChannels() == 2))) {
						int offs;
						for (offs=0; offs<4; offs++) destBuffer[offs] = (byte)headBuff[(hbPos+offs) & 3];
